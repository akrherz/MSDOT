...
def runner():
  for line in open('/mesonet/share/pickup/msdot/virtual_station.dat'):
    ...
    while now < ets:
      # We need to fast forward old data to next 40 years
      high_off = 0.01 * (now.year - 1970)
      low_off = 0.015 * (now.year - 1970)
      # Then we also add on climate change.
      delt = DELTAT[ now.month - 1] * (now.year - 2009) / 40.0
      high_off += delt
      low_off += delt
